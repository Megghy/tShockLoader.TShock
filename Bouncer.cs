	...
	internal sealed class Bouncer
	{
			...
			PlaceStyleCorrectors.Add(TileID.Torches,
				(player, requestedPlaceStyle, actualItemPlaceStyle) =>
				{
					...
					if (actualItemPlaceStyle == TorchID.Torch && player.unlockedBiomeTorches)
					{
						...
						var biomeTorchPlaceType = 0;
						var biomeTorchPlaceStyle = player.BiomeTorchPlaceStyle(ref biomeTorchPlaceType, ref actualItemPlaceStyle);
		...
		internal void OnTileEdit(object sender, GetDataHandlers.TileEditEventArgs args)
		{
			...
			try
			{
				...
				if (!args.Player.HasBuildPermission(tileX, tileY))
				{
					...
					{
						int itemFrameId = TEItemFrame.Find(tileX - tile.TileFrameX % 36 / 18, tileY - tile.TileFrameY % 36 / 18);
						if (itemFrameId != -1)
						{
				...
				if (action == EditAction.KillTile && Main.tile[tileX, tileY].TileType == TileID.MagicalIceBlock)
				{
				...
				if (action == EditAction.KillTile && !Main.tileCut[tile.TileType] && !breakableTiles.Contains(tile.TileType) && args.Player.RecentFuse == 0)
				{
					// If the tile is an axe tile and they aren't selecting an axe, they're hacking.
					if (Main.tileAxe[tile.TileType] && ((args.Player.TPlayer.mount.Type != MountID.Drill && selectedItem.axe == 0) && !Constants.Explosives[selectedItem.netID]))
					{
					...
					else if (Main.tileHammer[tile.TileType] && ((args.Player.TPlayer.mount.Type != MountID.Drill && selectedItem.hammer == 0) && !Constants.Explosives[selectedItem.netID]))
					{
					...
					else if (tile.TileType != TileID.ItemFrame && tile.TileType != TileID.MysticSnakeRope
														   && !Constants.Explosives[selectedItem.netID]
														   && !TileID.Sets.BreakableWhenPlacing[tile.TileType]
														   && !Main.tileAxe[tile.TileType] && !Main.tileHammer[tile.TileType] && tile.WallType == 0
				...
				else if (action == EditAction.KillWall)
				{
					...
					if (selectedItem.hammer == 0 && !Constants.Explosives[selectedItem.netID] && args.Player.RecentFuse == 0 && selectedItem.createWall == 0)
					{
				...
				if (TShock.Config.Settings.AllowCutTilesAndBreakables && Main.tileCut[tile.TileType])
				{
				...
				if (((action == EditAction.PlaceTile && editData != TileID.MysticSnakeRope) || (action == EditAction.KillTile && tile.TileType != TileID.MysticSnakeRope)) && !args.Player.IsInRange(tileX, tileY))
				{
					...
					if (action == EditAction.KillTile || action == EditAction.KillWall && Constants.Explosives[selectedItem.netID] && args.Player.RecentFuse == 0)
					{
						args.Handled = false;
						return;
					}
				...
				if (!((action == EditAction.PlaceTile && editData == TileID.MysticSnakeRope) || (action == EditAction.KillTile && tile.TileType == TileID.MysticSnakeRope)))
				{
					...
					if ((action == EditAction.KillTile || action == EditAction.KillTileNoItem || action == EditAction.ReplaceTile || action == EditAction.KillWall || action == EditAction.ReplaceWall) && Main.tileSolid[Main.tile[tileX, tileY].TileType] &&
		...
		private void GetRollbackRectSize(int tileX, int tileY, out byte width, out byte length, out int offsetY)
		{
			...
			void CheckForTileObjectsAbove(out byte objWidth, out byte objLength, out int yOffset)
			{
				...
				var above = Main.tile[tileX, tileY - 1];
				var tileObjectData = Utils.GetStaticPrivateFieldValue<List<TileObjectData>>(typeof(TileObjectData), "_data");
				if (above.TileType < tileObjectData.Count && tileObjectData[above.TileType] != null)
				{
					var data = tileObjectData[above.TileType];
			...
			void CheckForTileObjectsBelow(out byte objWidth, out byte objLength)
			{
				...
				var below = Main.tile[tileX, tileY + 1];
				var tileObjectData = Utils.GetStaticPrivateFieldValue<List<TileObjectData>>(typeof(TileObjectData), "_data");
				if (below.TileType < tileObjectData.Count && tileObjectData[below.TileType] != null)
				{
					var data = tileObjectData[below.TileType];
					objWidth = (byte)data.Width;
					objLength = (byte)data.Height;
				}
			}
		...
		internal void OnNewProjectile(object sender, GetDataHandlers.NewProjectileEventArgs args)
		{
			...
			*/
			var _fractalProfiles = Utils.GetStaticPrivateFieldValue<Dictionary<int, Terraria.Graphics.FinalFractalHelper.FinalFractalProfile>>(typeof(Terraria.Graphics.FinalFractalHelper), "_fractalProfiles");
			if (TShock.Config.Settings.DisableModifiedZenith && type == ProjectileID.FinalFractal && (ai[0] < -100 || ai[0] > 101) && !_fractalProfiles.ContainsKey((int)ai[1]))
			{
		...
		internal void OnPlaceChest(object sender, GetDataHandlers.PlaceChestEventArgs args)
		{
			...
			if (flag != 0 && flag != 4 // if no container or container2 placement
				&& Main.tile[tileX, tileY].TileType != TileID.Containers
				&& Main.tile[tileX, tileY].TileType != TileID.Dressers
				&& Main.tile[tileX, tileY].TileType != TileID.Containers2
				&& (!TShock.Utils.HasWorldReachedMaxChests() && Main.tile[tileX, tileY].TileType != TileID.Dirt)) //Chest
			{
			...
			{
				if ((TShock.Utils.TilePlacementValid(tileX, tileY + 1) && Main.tile[tileX, tileY + 1].TileType == TileID.Teleporter) ||
					(TShock.Utils.TilePlacementValid(tileX + 1, tileY + 1) && Main.tile[tileX + 1, tileY + 1].TileType == TileID.Teleporter))
				{
		...
		internal void OnPlaceObject(object sender, GetDataHandlers.PlaceObjectEventArgs args)
		{
			...
			else
			{
				...
				{
					var biomeTorchPlaceType = 0;
					var biomeCampfirePlaceType = 0;
					var validTorch = args.Player.SelectedItem.createTile == TileID.Torches && args.Player.TPlayer.BiomeTorchPlaceStyle(ref biomeTorchPlaceType, ref args.Player.SelectedItem.placeStyle) == style;
					var validCampfire = args.Player.SelectedItem.createTile == TileID.Campfire && args.Player.TPlayer.BiomeCampfirePlaceStyle(ref biomeCampfirePlaceType, ref args.Player.SelectedItem.placeStyle) == style;
		...
		internal void OnPlayerDamage(object sender, GetDataHandlers.PlayerDamageEventArgs args)
		{
			...
			*/
			if (TShock.Config.Settings.DisableCustomDeathMessages && id != args.Player.Index && reason.SourceOtherIndex != 16 &&
				(reason.SourcePlayerIndex == -1 || reason.SourceNPCIndex != -1 || reason.SourceOtherIndex != -1 || reason.CustomReason != null))
			{
		...
		internal void OnKillMe(object sender, GetDataHandlers.KillMeEventArgs args)
		{
			...
			if (playerDeathReason != null)
			{
				...
				if (TShock.Config.Settings.DisableCustomDeathMessages && playerDeathReason.CustomReason != null)
				{
