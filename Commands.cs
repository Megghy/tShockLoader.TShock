...
namespace TShockAPI
{
	...
	public static class Commands
	{
		...
		private static void MaxSpawns(CommandArgs args)
		{
			...
			{
				TShock.Config.Settings.DefaultMaximumSpawns = 5;
				Utils.SetStaticPrivateFieldValue(typeof(NPC), "defaultMaxSpawns", 5);
				if (args.Silent)
				{
			...
			Utils.SetStaticPrivateFieldValue(typeof(NPC), "defaultMaxSpawns", maxSpawns);
			if (args.Silent)
			{
		...
		private static void SpawnRate(CommandArgs args)
		{
			...
			if (String.Equals(args.Parameters[0], "default", StringComparison.CurrentCultureIgnoreCase))
			{
				TShock.Config.Settings.DefaultSpawnRate = 600;
				Utils.SetStaticPrivateFieldValue(typeof(NPC), "defaultSpawnRate", 600);
				if (args.Silent)
				{
			...
			Utils.SetStaticPrivateFieldValue(typeof(NPC), "defaultSpawnRate", spawnRate);
			if (args.Silent)
			{
						...
						if (displayBoundaries)
						{
							...
							foreach (Point boundaryPoint in Utils.Instance.EnumerateRegionBoundaries(regionArea))
							{
								...
								if ((boundaryPoint.X + boundaryPoint.Y & 1) == 0)
								{
									...
									var tile = Main.tile[boundaryPoint.X, boundaryPoint.Y];
									bool oldWireState = tile.RedWire;
									tile.RedWire = true;
		...
		private static void FireWork(CommandArgs args)
		{
			...
			else
			{
				...
				var target = players[0];
				int p = Projectile.NewProjectile(Projectile.GetSource_None(), target.TPlayer.position.X, target.TPlayer.position.Y - 64f, 0f, -8f, type, 0, 0);
		...
		private static void Item(CommandArgs args)
		{
			...
			if (args.Player.InventorySlotAvailable || (item.type > ItemID.WormFood && item.type < ItemID.FallenStar) || item.ammo > 0 || item.type == ItemID.Heart || item.type == ItemID.Star)
			{
		...
		private static void RenameNPC(CommandArgs args)
		{
			...
			for (int i = 0; i < Main.npc.Length; i++)
			{
				if (Main.npc[i].active && ((npcId == 0 && !Main.npc[i].townNPC) || (Main.npc[i].netID == npcId && Main.npc[i].townNPC)))
				{
					...
					NetMessage.SendData(MessageID.UniqueTownNPCInfoSyncRequest, -1, -1, NetworkText.FromLiteral(args.Parameters[1]), i, 0f, 0f, 0f, 0);
					done++;
				}
			}
			if (done > 0)
		...
		private static void Give(CommandArgs args)
		{
			...
			else
			{
				...
				if (item.type >= ItemID.IronPickaxe && item.type < Terraria.ID.ItemID.Count)
				{
					...
					{
						var plr = players[0];
						if (plr.InventorySlotAvailable || (item.type > ItemID.WormFood && item.type < ItemID.FallenStar) || item.ammo > 0 || item.type == ItemID.Heart || item.type == ItemID.Star)
						{
							if (itemAmount == 0 || itemAmount > item.maxStack)
								itemAmount = item.maxStack;
			...
			switch (subcmd)
			{
				case "help":
					...
					bool prepareAreaForGrow(ushort groundType = TileID.Grass, bool evil = false)
					{
						...
						{
							var tile = Main.tile[i, y];
							tile.HasTile = true;
							tile.TileType = groundType;
							tile.WallType = WallID.None;
						}
						var tile2 = Main.tile[x, y - 1];
						tile2.WallType = WallID.None;
					...
					bool growPalmTree(ushort sandType, ushort supportingType, string properName, bool evil = false)
					{
						...
						for (int i = x - 2; i < x + 3; i++)
						{
							...
							tile1.HasTile = true;
							tile1.TileType = sandType;
							tile1.WallType = WallID.None;
						}
						for (int i = x - 2; i < x + 3; i++)
						{
							Tile tile2 = Main.tile[i, y + 1];
							tile2.HasTile = true;
							tile2.TileType = supportingType;
							tile2.WallType = WallID.None;
						}

						Tile tile3 = Main.tile[x, y - 1];
						tile3.WallType = WallID.None;
